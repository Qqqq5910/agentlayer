...
export function evaluateTasks(
  ...
  return tasks.map((task) =>
    AgentTaskResultSchemaLike(parseTaskResult(task, scan, facts, actions))
  );
}
...
function parseTaskResult(
  ...
  switch (task.id) {
    ...
    case "book_demo":
      return evaluateActionPath(
        task,
        scan,
        actions,
        ["book_demo", "contact_sales", "request_quote"],
        ["demo", "sales", "contact"]
      );
    case "find_docs":
      return evaluatePageAndAction(
        task,
        scan,
        facts,
        actions,
        ["docs", "api_docs"],
        ["docs", "open_api_docs", "search_docs"]
      );
    case "find_security":
      return evaluatePageAndAction(
        task,
...
function evaluateFindPricing(
  ...
  const pricingActions = actions.filter((action) =>
    ["view_pricing", "contact_sales", "request_quote"].includes(action.name)
  );
  const evidence = collectEvidence(
    [...pricingPages, ...pricingFacts, ...pricingActions],
    task.requiredEvidence
  );
  const hasPlansOrPrice = pricingFacts.some(
    (fact) => fact.type === "plan" || /(?:\$|\bprice|\bplan)/i.test(fact.value)
  );
  ...
  if (pricingPages.length > 0 && hasPlansOrPrice) {
    return result(
      ...
      "pass",
      90,
      "Pricing information is discoverable with page-level evidence.",
      evidence
    );
  }
  if (pricingPages.length > 0 || pricingActions.length > 0 || pricingFacts.length > 0) {
    return result(
      ...
      "partial",
      60,
      "A pricing path exists, but plan or price details are incomplete.",
      evidence,
  ...
  return result(
    ...
    "fail",
    0,
    "No pricing page, pricing fact, or quote/contact-sales pricing path was found.",
    evidence,
    ["Create a pricing page or add a clear pricing/contact-sales link."]
  );
}

function evaluateComparePlans(
  task: AgentTask,
  scan: SiteScan,
  facts: readonly ExtractedFact[]
): AgentTaskResult {
  ...
  const pricingPages = pagesOfType(scan, ["pricing"]);
  const featureEvidence = pricingPages.filter((page) =>
    includesAny(page.visibleText, ["features", "limits", "seats", "included"])
  );
  ...
  if (planFacts.length >= 2 && featureEvidence.length > 0) {
    return result(
      ...
      "pass",
      85,
      "Multiple plans and comparison-oriented details were found.",
      evidence
    );
  }
  if (planFacts.length > 0 || pricingPages.length > 0) {
    return result(
      ...
      "partial",
      50,
      "Plan evidence exists, but differences are not fully machine-readable.",
      evidence,
...
function evaluatePolicies(
  ...
  const policyActions = actions.filter((action) =>
    ["read_privacy_policy", "read_terms"].includes(action.name)
  );
  const evidence = collectEvidence(
    [...policyPages, ...policyFacts, ...policyActions],
    task.requiredEvidence
  );
  ...
  if (policyKinds.has("privacy") && policyKinds.has("terms")) {
    return result(
      ...
      "pass",
      85,
      "Privacy and terms/legal policy pages are discoverable.",
      evidence
    );
  }
  if (policyPages.length > 0 || policyFacts.length > 0 || policyActions.length > 0) {
    return result(
      ...
      "partial",
      55,
      "Some policy evidence was found, but coverage is incomplete.",
      evidence,
  ...
  return result(
    ...
    "fail",
    0,
    "No privacy, terms, cancellation, or refund policy evidence was found.",
    evidence,
    ["Add visible policy links and machine-readable summaries."]
  );
}

function evaluateTargetCustomer(
  task: AgentTask,
  scan: SiteScan,
  facts: readonly ExtractedFact[]
): AgentTaskResult {
  const targetFacts = facts.filter(
    (fact) => fact.type === "product" && /target|user|customer/i.test(fact.label)
  );
  const targetPages = scan.pages.filter((page) =>
    includesAny(page.visibleText, [
      "for teams",
      "for developers",
      "for enterprises",
      "for founders",
  ...
  if (targetFacts.length > 0 || targetPages.length > 0) {
    return result(
      task,
      "pass",
      targetFacts.length > 0 ? 80 : 70,
      "Target user evidence was found.",
      evidence
    );
  }
...
function evaluateActionPath(
  ...
  const matchingPages = scan.pages.filter((page) =>
    includesAny(`${page.pageType} ${page.finalUrl} ${page.visibleText}`, pageKeywords)
  );
  ...
  if (hasForm) {
    return result(
      ...
      "pass",
      90,
      "A relevant form-based action path was found.",
      evidence,
      [],
      journeySteps
    );
  ...
  return result(
    ...
    "fail",
    0,
    "No matching action path was found.",
    evidence,
...
function evaluatePageAndAction(
  ...
  const evidence = collectEvidence(
    [...pages, ...matchingFacts, ...matchingActions],
    task.requiredEvidence
  );
  ...
  if (pages.length > 0 || matchingActions.length > 0 || matchingFacts.length > 0) {
    return result(
      ...
      "partial",
      60,
      "Some relevant evidence was found, but it is incomplete.",
      evidence,
...
function evaluatePageAndFact(
  ...
  const matchingFacts = facts.filter(
    (fact) =>
      factTypes.includes(fact.type) || keywords.some((keyword) => factMatches(fact, keyword))
  );
  ...
  if (pages.length > 0 || matchingFacts.length > 0) {
    return result(
      ...
      "partial",
      55,
      "Some relevant evidence was found, but details are incomplete.",
      evidence,
...
function evaluateGenericTask(
  ...
  const pageMatches = scan.pages.filter(
    (page) =>
      countKeywordHits(
        `${page.title ?? ""} ${page.visibleText} ${page.finalUrl}`,
        task.requiredEvidence
      ) > 0
  );
  ...
  const evidence = collectEvidence(
    [...pageMatches, ...factMatchesForTask, ...actionMatches],
    task.requiredEvidence
  );
  const score = clamp(
    Math.round((evidence.snippets.length / Math.max(1, task.requiredEvidence.length)) * 100),
    0,
    100
  );
  ...
  return result(
    ...
    score,
    score > 0
      ? "Keyword evidence was found for this custom task."
      : "No keyword evidence was found for this custom task.",
    evidence,
...
function collectEvidence(
  items: readonly (PageSnapshot | ExtractedFact | AgentAction)[],
  keywords: readonly string[]
): Evidence {
  const urls: string[] = [];
  ...
  for (const item of items) {
    if ("finalUrl" in item) {
      ...
      snippets.push(
        truncateText(
          `${item.label}: ${item.value}${item.sourceText ? ` - ${item.sourceText}` : ""}`,
          240
        )
      );
    } else if ("url" in item) {
...
function actionJourneySteps(
  ...
  const evidenceUrls = uniqueStrings(
    [firstAction?.url, firstAction?.sourceUrl, firstPage?.finalUrl].filter(Boolean) as string[]
  );
  ...
  const sensitiveActionNeedsConfirmation = firstAction
    ? firstAction.sensitivity !== "low" || firstAction.actionType === "form"
    : false;
  ...
  return [
    {
      ...
      evidenceUrls,
      evidenceSnippets: firstAction
        ? [truncateText(`${firstAction.name}: ${firstAction.description}`, 180)]
        : []
    },
    {
...
function factMatches(fact: ExtractedFact, keyword: string): boolean {
  return includesAny(`${fact.type} ${fact.label} ${fact.value} ${fact.sourceText ?? ""}`, [
    keyword
  ]);
}
