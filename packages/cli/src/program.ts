...
export function createProgram(io: CliIo = defaultIo): Command {
  ...
  program
    .name("agentlayer")
    .description("Deterministic CLI for scanning and generating AgentLayer artifacts for agent-operable websites.")
...
`
  ...
  program
    ...
    .option("--timeout-ms <number>", "Per-request timeout in milliseconds.", parsePositiveInteger, 10000)
    .option("--json", "Print machine-readable JSON to stdout.")
    .action((url: string, options) => runScanCommand(url, options, io));

  program
    .command("generate")
    .description("Scan a website, build a report, and write generated AgentLayer artifacts recursively.")
    .argument("<url>", "Root http(s) URL to scan.", parseHttpUrl)
    ...
    .option("--json", "Print machine-readable JSON to stdout.")
    .action((url: string, options) => runGenerateCommand(url, options, io));
  ...
  program
    ...
    .option("--tasks <path>", "JSON task suite to use instead of the default B2B SaaS suite.")
    .option("--json", "Print machine-readable JSON to stdout.")
    .action((url: string, options) => runTestCommand(url, options, io));

  program
    .command("doctor")
    .description("Print a quick AgentLayer readiness diagnosis for a website.")
    .argument("<url>", "Root http(s) URL to scan.", parseHttpUrl)
    .option("--out <path>", "Optional output directory or .json file path for the doctor diagnosis.")
    .option("--max-pages <number>", "Maximum same-host pages to scan.", parsePositiveInteger, 20)
    .option("--timeout-ms <number>", "Per-request timeout in milliseconds.", parsePositiveInteger, 10000)
    .option("--tasks <path>", "JSON task suite to use instead of the default B2B SaaS suite.")
    .option("--json", "Print machine-readable JSON to stdout.")
    .action((url: string, options) => runDoctorCommand(url, options, io));
  ...
  program
    ...
    .option("--json", "Print machine-readable JSON to stdout.")
    .action((options) => runInitFixtureCommand(options, io));
