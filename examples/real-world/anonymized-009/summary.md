# anonymized-009 Real-World Scan Summary

This is an anonymized, bounded public-site scan summary. It intentionally excludes raw page text,
private URLs, credentials, customer data, internal paths, screenshots, and unpublished plans.

- Sample ID: anonymized-009
- Public root URL scanned: redacted in committed sample
- Site type: Search, observability, and security platform
- Scan status: completed
- Pages scanned: 5
- Overall score: 71/100
- Task success score: 72.78/100
- Scope note: Public pages only. No forms were submitted. No authenticated, private, local, or
  internal URLs were scanned.

## Pages Scanned

- home /: HTTP 200
- security /trust/ai-data-faq/supplemental-terms: HTTP 200
- pricing /blog/advantages-of-resource-based-pricing-in-security: HTTP 200
- integrations /blog/security-integration-example: HTTP 200
- unknown /blog/building-customer-trust-through-secure-experiences-with-open-banking: HTTP 200

## Task Results

- pass: Find pricing; Compare plans; Find security and trust information; Find integrations;
  Identify target customer
- partial: Book demo or contact sales; Find documentation; Find policies; Find support
- fail: none
- unavailable: none

## Crawl Issues

- none recorded

## Wrong Facts Or Actions Observed

- Several blog article URLs were classified as pricing, security, or integrations pages based on
  path/title keywords; this is likely a classifier false positive candidate.
- Demo/contact-sales was often partial because the scanner found navigation but did not detect an
  actionable form in the bounded public crawl.

## Confusing Recommendations

- Add /llms.txt
- Improve task: Book demo or contact sales
- Improve task: Find documentation
- Improve task: Find policies
- Improve task: Find support
- Book demo or contact sales: Expose field labels and purpose for the relevant form.
- Find documentation: Add clearer page titles, headings, and stable links for this journey.
- Find policies: Expose privacy, terms, cancellation, and refund policies from stable URLs.
- Find support: Expose field labels and purpose for the relevant form.

## Artifacts I Would Publish

- scan summary
- task-status summary
- heuristic issue notes
- doctor score snapshot
- page inventory without raw text
- pass/partial/fail task table

## Notes

- Public pages only.
- Keep raw CLI output in temporary local storage only.
- Treat this summary as heuristic feedback, not as a review of the scanned company or product.
