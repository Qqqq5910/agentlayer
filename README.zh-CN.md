...
然后访问 `http://localhost:3000/demo`。如果想跑完整扫描，可以启动 AcmeFlow
fixture，再生成文件并打开输出目录中的 `report.html`：
...
上方截图是当前生成文件预览。发布前可以直接使用 `docs/assets/agentlayer-preview.svg` 作为 social
preview，也可以从 `http://localhost:3000/demo` 重新截取一张 demo 截图。
...
它不是 AI SEO 面板，也不是单纯的 `llms.txt` 生成器。更准确地说，AgentLayer 像面向 Agentic
...
- Release：用 [release-notes/v0.1.0.md](./release-notes/v0.1.0.md) 创建 `v0.1.0` release。
...
- v0.1 的 CLI 主要按仓库内 `pnpm agentlayer` 使用方式文档化。
